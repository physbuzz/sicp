...
class ComposableMarkdownExtension(Extension):
    def __init__(self, **kwargs):
        self.config = {
            ...
            'show_filename': [False, 'Show the filename above code blocks'],
...
class ComposableMarkdownPreprocessor(Preprocessor):
    ...
    INCLUDE_PATTERN = re.compile(r'@src\(([\w\-\./]+\.\w+)\)')
    ...
    def _process_lines(self, lines, current_path=None):
        ...
        while i < len(lines):
            ...
            if match:
                ...
                processed_lines = self._process_code_file(file_path, "racket")
                new_lines.extend(processed_lines)
            
            # Check for @import(file.ext) pattern
            elif self.INCLUDE_PATTERN.search(line):
                ...
                processed_lines = self._process_code_file(file_path, lang)
                new_lines.extend(processed_lines)
    ...
    def _process_code_file(self, file_path, language):
        ...
        try:
            ...
            # Add filename above code block if configured
            if self.show_filename:
                result.append(f"<div class='code-filename'>{file_path}</div>")
            ...
            if self.run_link and language == "racket":
